...
const CAT_CONFIGS = [
  ...
  },
  {
    ...
    label: "Category 2",
    badge: "Easy",
    placeholder: "e.g. ___ Ball",
  },
  {
    cls: "b",
    label: "Category 3",
    badge: "Medium",
    placeholder: "e.g. Things that can fly",
  },
  {
    cls: "p",
    label: "Category 4",
    badge: "Hard",
    placeholder: "e.g. ___ Stone",
  },
  {
    cls: "r",
    label: "Category 5",
    badge: "Harder",
    placeholder: "e.g. Famous Painters",
  },
  {
    cls: "o",
    label: "Category 6",
    badge: "Hardest",
    placeholder: "e.g. ___ Fire",
  },
];
...
function bindCreatorButtons() {
  ...
  // Category stepper
  replaceListener("num-cats-dec", () => {
    if (numCategories > 2) {
      numCategories--;
      renderCreatorCards();
    }
  });
  replaceListener("num-cats-inc", () => {
    if (numCategories < 6) {
      numCategories++;
      renderCreatorCards();
    }
  ...
  replaceListener("words-per-cat-dec", () => {
    if (wordsPerCat > 2) {
      wordsPerCat--;
      renderCreatorCards();
    }
  });
  replaceListener("words-per-cat-inc", () => {
    if (wordsPerCat < 6) {
      wordsPerCat++;
      renderCreatorCards();
    }
...
async function handleGenerate() {
  ...
  for (let ci = 0; ci < numCategories; ci++) {
    ...
    if (words.length !== wordsPerCat) {
      showToast(
        `Category ${ci + 1} needs ${wordsPerCat} words (${words.length} filled).`,
      );
      valid = false;
      break;
    }
